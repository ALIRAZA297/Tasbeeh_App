...
//       } else {
//         Get.snackbar("Error", "Failed to fetch Quran data",
  ...
  Future<void> fetchQuranData() async {
    ...
    if (savedQuran != null) {
      try {
        ...
        print("Surahs loaded from cache: ${quranModel.value.surahs.length}");
        isLoading.value = false;
        return; // Exit early since we loaded from cache
      } catch (e) {
        print("Error loading cached Quran data: $e");
    ...
    try {
      print("Fetching from API: ${ApiService.quranApi}");
      final response = await http.get(Uri.parse(ApiService.quranApi));

      if (response.statusCode == 200) {
        quranModel.value = QuranModel.fromJson(json.decode(response.body));
        print("Surahs loaded from API: ${quranModel.value.surahs.length}");
        // Save to SharedPreferences after successful fetch
        await prefs.setString('quranData', jsonEncode(quranModel.value.toJson()));
        print("Quran data saved to SharedPreferences");
      } else {
